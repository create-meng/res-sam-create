"""
V30 parallel utilities structural-mechanism sweep runner.

用途：
- 将 V30 的 5 个固定机制 case 拆成两路并行
- 共享同一个 feature bank
...
SCRIPT_07 = BASE_DIR / "experiments" / "07_run_v30_quick_sweep.py"
BANK_PATH = BASE_DIR / "outputs" / "feature_banks_v30" / "feature_bank_v30.pth"
...
def main() -> int:
    ...
    env_a["V30_CASE_INDICES"] = "0,1,2"
    env_a["BUILD_BANK"] = "auto"
    print(f"[part_a] >>> {' '.join(cmd)}  (V30_CASE_INDICES={env_a['V30_CASE_INDICES']}, BUILD_BANK={env_a['BUILD_BANK']})")
    ...
    try:
        ...
        env_b["V30_CASE_INDICES"] = "3,4"
        env_b["BUILD_BANK"] = "0"
        print(f"[part_b] >>> {' '.join(cmd)}  (V30_CASE_INDICES={env_b['V30_CASE_INDICES']}, BUILD_BANK={env_b['BUILD_BANK']})")
