"""
Res-SAM v22 - Step 1: build the feature bank
...
环境变量：
- USE_ROTATION_AUG: 是否使用旋转增强Memory Bank (0/1, 默认0)
- ROTATION_ANGLES: 旋转角度，逗号分隔 (默认"90,180,270")
...
CONFIG = {
    ...
    "output_dir": os.path.join(BASE_DIR, "outputs", "feature_banks_v22"),
    "output_file": "feature_bank_v22.pth",
    ...
    # 旋转增强（新增）
    "use_rotation_aug": int(os.getenv("USE_ROTATION_AUG", "0")),
    "rotation_angles": [int(x) for x in os.getenv("ROTATION_ANGLES", "90,180,270").split(",")],
...
def build_feature_bank(config: dict):
    ...
    print("=" * 60)
    print("Res-SAM v22：Feature Bank 构建（基于 V21 最优配置）")
    print("=" * 60)
    ...
    print(f"\nFeature Bank v22 保存至: {output_path}")
...
if __name__ == "__main__":
    logger = setup_global_logger(BASE_DIR, "01_build_feature_bank_v22")
    
    preflight_faiss_or_raise()
    CONFIG = apply_layout_to_config_01(dict(CONFIG), BASE_DIR, "v22")
    CONFIG["version"] = "v22"
    ...
    if use_rotation:
        rotation_angles = CONFIG.get("rotation_angles", [90, 180, 270])
        angles_str = "_".join(map(str, rotation_angles))
        suffix = f"_rotation_{angles_str}"
        CONFIG["output_dir"] = os.path.join(BASE_DIR, "outputs", f"feature_banks_v22{suffix}")
        CONFIG["output_file"] = f"feature_bank_v22{suffix}.pth"
    else:
        CONFIG["output_dir"] = os.path.join(BASE_DIR, "outputs", "feature_banks_v22")
        CONFIG["output_file"] = "feature_bank_v22.pth"
    ...
    log_finish("01_build_feature_bank_v22", logger)
