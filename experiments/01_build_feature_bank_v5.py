"""
Res-SAM 复现实验 V5 - Step 1: Feature Bank 构建
...
CONFIG = {
    ...
    'output_dir': os.path.join(BASE_DIR, 'outputs', 'feature_banks_v5'),
    'output_file': 'feature_bank_v5.pth',
    ...
    # Resize policy: experiments/resize_policy.py
    # voc_annotation: augmented_intact 无 VOC → 按各图像原分辨率提取 patch（与环境增强集一致）。
    ...
    # 版本标识
    'version': 'V5',
    'alignment_notes': 'Enhanced-eval setup with domain-matched feature bank: source=augmented_intact, eval=augmented_*, feature f=[W_out,b]',
}
...
def load_normal_images(
    ...
    tuple: (images, paths, hashes)
        images is np.ndarray [N,H,W] if fixed resize; else list of [H,W] arrays.
    """
...
def build_feature_bank(config: dict, resume: bool = True):
    ...
    print("=" * 60)
    print("Res-SAM V5: Feature Bank Construction (Strict Paper Alignment)")
    print("=" * 60)
    ...
    all_metadata = {
        ...
        'preprocess_signature': {
            ...
            'resize': {
                ...
                'note':
                    'voc_annotation without VOC: native per-image HW; '
    ...
    print(f"\nFeature Bank V5 保存至: {output_path}")
...
if __name__ == "__main__":
    CONFIG = apply_layout_to_config_01(dict(CONFIG), BASE_DIR, "v5")
