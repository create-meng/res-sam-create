"""Res-SAM 复现实验 V4 - Click-guided 模式推理
...
- 输出到 outputs/predictions_v4/click_predictions_v4.json
...
def run_click_guided(config: dict):
    ...
    log_file = os.path.join(log_dir, f'click_inference_v4_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    ...
    logger.info('=' * 60)
    logger.info('Res-SAM V4: Click-guided Inference (Strict Paper Alignment)')
    logger.info('=' * 60)
    ...
    for click_cfg in config["click_configs"]:
        ...
        for category, data_dir in config["test_data_dirs"].items():
            ...
            category_key = f"{click_key}_{category}_v4"
            ...
            safe_category_key = f"click_{_normalize_cfg_for_path(cfg_name)}_{category}_v4"
    ...
    output_path = os.path.join(config["output_dir"], "click_predictions_v4.json")
