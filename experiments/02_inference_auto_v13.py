"""
Res-SAM v13 - Step 2: fully automatic inference.

V13 相对 V12 的改进：
1. GPR 背景去除预处理：推理时对每张图做水平背景滤波，与建库预处理一致
2. merge_all_anomaly_patches=True（继承 V12 验证有效的改动）
3. Feature Bank 使用 V13 coreset 采样建库
"""
...
def _require_segment_anything() -> None:
    try:
        ...
        print("错误：未安装 segment-anything\n请先安装：pip install segment-anything", flush=True)
        raise SystemExit(1)
...
CONFIG = {
    ...
    "feature_bank_path": os.path.join(BASE_DIR, "outputs", "feature_banks_v13", "feature_bank_v13.pth"),
    "metadata_path": os.path.join(BASE_DIR, "outputs", "feature_banks_v13", "metadata.json"),
    "test_data_dirs": {
    ...
    "output_dir": os.path.join(BASE_DIR, "outputs", "predictions_v13"),
    "checkpoint_dir": os.path.join(BASE_DIR, "outputs", "checkpoints_v13"),
    "window_size": 50,
    "stride": 5,
    "hidden_size": 30,
    "beta_threshold": DEFAULT_BETA_THRESHOLD,
    # V12 验证有效：F1 0.060→0.137，Precision 0.041→0.148，FP 284→75
    "merge_all_anomaly_patches": True,
    # V13 新增：GPR 背景去除，与建库预处理保持一致
    ...
    "alignment_notes": (
        "v13: PatchCore coreset建库 + GPR背景去除 + merge_all(beta=0.1); "
        "fb_source=augmented_intact全量coreset, eval=augmented_intact + annotated anomaly sets"
    ),
}
...
def run_inference(config: dict) -> dict:
    ...
    log_file = os.path.join(log_dir, f"auto_inference_v13_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
            ...
            for i in tqdm(range(start_idx, len(image_files)), desc=f"处理 {category}", file=sys.stdout):
                ...
                try:
                    ...
                    record = {
                        ...
                        "pred_bboxes": pred_bboxes, "anomaly_scores": anomaly_scores,
                    ...
                    if gt:
                        ...
                        gt_boxes_resized = [[int(b[0]*inv_scale_x), int(b[1]*inv_scale_y),
                                            ...
                                            for b in gt["bboxes"]]
                        record.update({"gt_bboxes": gt["bboxes"], "gt_bboxes_resized": gt_boxes_resized,
    ...
    output_file = os.path.join(config["output_dir"], "auto_predictions_v13.json")
    output_data = {
        "meta": {
            "version": "v13", "alignment_notes": config["alignment_notes"],
            "creation_time": datetime.now().isoformat(),
            "feature_bank_path": config["feature_bank_path"],
            "window_size": int(config["window_size"]), "stride": int(config["stride"]),
...
if __name__ == "__main__":
    ...
    CONFIG = apply_layout_to_config_02_03(dict(CONFIG), BASE_DIR, "v13")
    CONFIG["feature_bank_path"] = os.path.join(BASE_DIR, "outputs", "feature_banks_v13", "feature_bank_v13.pth")
    CONFIG["metadata_path"] = os.path.join(BASE_DIR, "outputs", "feature_banks_v13", "metadata.json")
    CONFIG["output_dir"] = os.path.join(BASE_DIR, "outputs", "predictions_v13")
    CONFIG["checkpoint_dir"] = os.path.join(BASE_DIR, "outputs", "checkpoints_v13")
    with torch.no_grad():
