"""
Res-SAM V7 - Step 1: build the feature bank.

Current V7 mainline policy:
...
For preprocessing, V7 uses a fixed resize to 369x369 for all images,
matching the default loading convention in the official repository.
The V7 runtime seed is unified to 11.
"""
...
CONFIG = {
    ...
    "output_dir": os.path.join(BASE_DIR, "outputs", "feature_banks_v7"),
    "output_file": "feature_bank_v7.pth",
    ...
    "alignment_notes": "Paper-first mainline (V7): solve true f=[W_out,b]; fb_source=augmented_intact, eval=augmented_*",
}
...
def build_feature_bank(config: dict, resume: bool = True):
    ...
    print("=" * 60)
    print("Res-SAM V7：Feature Bank 构建")
    print("=" * 60)
    ...
    all_metadata = {
        ...
        "preprocess_signature": {
            ...
            "resize": {
                ...
                "note": "V7 mainline uses fixed resize; all images are resized to image_size to match the official 369x369 default.",
            },
    ...
    print(f"\nFeature Bank V7 保存至: {output_path}")
...
if __name__ == "__main__":
    ...
    # 复用当前唯一数据布局，并固定输出到 v7 Feature Bank 目录。
    CONFIG = apply_layout_to_config_01(dict(CONFIG), BASE_DIR, "v7")
    CONFIG["version"] = "V7"
    CONFIG["output_dir"] = os.path.join(BASE_DIR, "outputs", "feature_banks_v7")
    CONFIG["output_file"] = "feature_bank_v7.pth"
