"""
Res-SAM V7 - Step 2: fully automatic inference.

This script implements the V7 automatic mainline corresponding to the
paper's fully automatic mode. Feature definition follows f=[W_out, b].
Image preprocessing is unified to a fixed 369x369 resize, and the V7
...
CONFIG = {
    ...
    "feature_bank_path": os.path.join(BASE_DIR, "outputs", "feature_banks_v7", "feature_bank_v7.pth"),
    "metadata_path": os.path.join(BASE_DIR, "outputs", "feature_banks_v7", "metadata.json"),
    "test_data_dirs": {
    ...
    "output_dir": os.path.join(BASE_DIR, "outputs", "predictions_v7"),
    "checkpoint_dir": os.path.join(BASE_DIR, "outputs", "checkpoints_v7"),
    "window_size": 50,
    "stride": 5,
    "hidden_size": 30,
    ...
    "alignment_notes": (
        "V7 experiment A: solve true f=[W_out,b]; keep region-feature coarse filtering, but use mask-constrained fine-stage sampling; "
...
def run_inference(config: dict) -> dict:
    ...
    log_file = os.path.join(log_dir, f"auto_inference_v7_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    if not os.path.exists(config["feature_bank_path"]):
        raise FileNotFoundError(
            ...
            "Please run 01_build_feature_bank_v7.py first."
        )
    ...
    version_tag = str(config.get("version", "V7")).strip().lower()
    ...
    output_data = {
        "meta": {
            ...
            "image_preprocess_note": ("V7 mainline uses fixed resize; all images are resized to fixed_image_size_hw before SAM, 2D-ESN, and evaluation coordinate mapping."),
...
if __name__ == "__main__":
    ...
    CONFIG = apply_layout_to_config_02_03(dict(CONFIG), BASE_DIR, "v7")
    with torch.no_grad():
