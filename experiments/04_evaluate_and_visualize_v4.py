...
CONFIG = {
    ...
    'predictions_path': os.path.join(
        ...
        'outputs',
        'predictions_v4',
        'auto_predictions_v4.json',
    ),

    # Click-guided 结果路径 (V4 / Table 1)
    'click_predictions_path': os.path.join(
        BASE_DIR,
        'outputs',
        'predictions_v4',
        'click_predictions_v4.json',
    ),
    ...
    'analysis_output': os.path.join(
        ...
        'outputs',
        'visualizations_v4',
        '04_evaluate_and_visualize_v4_report.md',
    ),
...
def plot_roc_curve(results: dict, output_path: str):
    ...
    plt.title('Res-SAM V4 ROC Curve (Fully Automatic Mode)', fontsize=14)
...
def main():
    ...
    if os.path.exists(CONFIG['predictions_path']):
        ...
        # 绘制 ROC 曲线
        print("\n绘制 ROC 曲线...")
        roc_path = os.path.join(CONFIG['vis_output_dir'], 'roc_curve_v4.png')
        plot_roc_curve(results, roc_path)

        # 阈值分析
        print("\n阈值优化分析...")
        thresh_path = os.path.join(CONFIG['vis_output_dir'], 'threshold_analysis_v4.png')
        ...
        print("跳过 automatic 评估：请先运行 02_inference_auto_v4.py")
    ...
    if click_path and os.path.exists(click_path):
        ...
        print("跳过 click 评估：请先运行 03_inference_click_v4.py")
