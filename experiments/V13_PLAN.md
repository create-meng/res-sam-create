...
### 增强1：PatchCore Coreset 采样建库（01_build_feature_bank_v13.py）

**问题**：V12 随机/分组采样 75 张，Feature Bank 覆盖不足，正常图被误判为高分异常
（正常图 mean_score=0.551 > 异常图 0.318~0.403）

**改动**：
- 加载全量 augmented_intact（900 张）的所有 patch 特征
- 用 PatchCore 贪心 coreset 采样，选出最具代表性的子集（约 9000 个 patch，coreset_ratio=0.01）
- 保证 Feature Bank 覆盖所有正常纹理变体，不遗漏任何原始图
...
```bash
# 建库（全量900张 → coreset ~9000 patch，约需5~10分钟）
...
| V13（预期） | >0.15 | >0.75 | coreset + 背景去除 + merge_all |
