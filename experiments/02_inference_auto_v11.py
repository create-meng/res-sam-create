"""
Res-SAM v11 - Step 2: fully automatic inference.

This script implements the v11 automatic mainline corresponding to the
paper's fully automatic mode. Feature definition follows f=[W_out, b].
Image preprocessing is unified to a fixed 369x369 resize, and the v11
...
from experiments.paper_constants import DEFAULT_BETA_THRESHOLD, DEFAULT_COARSE_BETA_V11, DEFAULT_FINE_BETA_V11, preflight_faiss_or_raise
...
CONFIG = {
    ...
    "metadata_path": os.path.join(BASE_DIR, "outputs", "feature_banks_v11", "metadata.json"),
    "test_data_dirs": {
    ...
    "output_dir": os.path.join(BASE_DIR, "outputs", "predictions_v11"),
    "checkpoint_dir": os.path.join(BASE_DIR, "outputs", "checkpoints_v11"),
    "window_size": 50,
    "stride": 5,
    "hidden_size": 30,
    "beta_threshold": DEFAULT_FINE_BETA_V11,       # V11 细筛beta：p99校准值0.183，严格减少FP
    "coarse_beta_threshold": DEFAULT_COARSE_BETA_V11,  # V11 粗筛beta：0.05宽松，不漏真异常region
    ...
    "merge_all_anomaly_patches": True,        # 所有异常patch合并为单一bbox
    "num_normal_samples": 100,                # 方案C：增加Feature Bank样本数（论文20，V11用100）
    "alignment_notes": (
        "V11: coarse_beta=0.05(loose) + fine_beta=0.183(p99) + merge_all + num_normal_samples=100; "
        "fixes: split coarse/fine beta to avoid discarding true anomaly regions in coarse stage; "
...
def run_inference(config: dict) -> dict:
    ...
    log_file = os.path.join(log_dir, f"auto_inference_v11_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    model = ResSAM(
        ...
        coarse_beta_threshold=config.get("coarse_beta_threshold", None),
    )
    ...
    version_tag = str(config.get("version", "v11")).strip().lower()
    ...
    output_data = {
        "meta": {
            ...
            "image_preprocess_note": ("v11 mainline uses fixed resize; all images are resized to fixed_image_size_hw before SAM, 2D-ESN, and evaluation coordinate mapping."),
...
if __name__ == "__main__":
    ...
    CONFIG = apply_layout_to_config_02_03(dict(CONFIG), BASE_DIR, "v11")
    with torch.no_grad():
