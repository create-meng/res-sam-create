"""
Res-SAM 复现实验 V5 - Step 2: Fully Automatic 模式推理
...
CONFIG = {
    ...
    "metadata_path": os.path.join(
        BASE_DIR, "outputs", "feature_banks_v5", "metadata.json"
    ),
    # 测试数据
    "test_data_dirs": {
    ...
    "output_dir": os.path.join(
        BASE_DIR, "outputs", "predictions_v5"
    ),
    "checkpoint_dir": os.path.join(
        BASE_DIR, "outputs", "checkpoints_v5"
    ),
    # 论文参数（V3 严格对齐）
    "window_size": 50,  # 论文默认值
    "stride": 5,
...
def run_inference(config: dict):
    ...
    log_file = os.path.join(log_dir, f"auto_inference_v5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    print("=" * 60)
    print("Res-SAM V5: Fully Automatic Inference (Strict Paper Alignment)")
    print("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info("Res-SAM V5: Fully Automatic Inference (Strict Paper Alignment)")
    logger.info("=" * 60)
    ...
    # 保存结果
    output_file = os.path.join(config["output_dir"], "auto_predictions_v5.json")
