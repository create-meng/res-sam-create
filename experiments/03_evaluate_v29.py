...
def evaluate_v29(pred_path, meta_path):
    """评估 v29 结果"""
    try:
...
if __name__ == '__main__':
    logger = setup_global_logger(BASE_DIR, "03_evaluate_v29")
    log_section("V29 评估：主线版本", logger)

    output_suffix = _normalize_suffix("OUTPUT_SUFFIX", "")
    bank_suffix = _normalize_suffix("BANK_SUFFIX", "")
    pred_filename = f"auto_predictions_v29{output_suffix}.json" if output_suffix else "auto_predictions_v29.json"
    report_filename = f"evaluation_report_v29{output_suffix}.json" if output_suffix else "evaluation_report_v29.json"
    pred_path   = os.path.join(BASE_DIR, "outputs", "predictions_v29", pred_filename)
    report_path = os.path.join(BASE_DIR, "outputs", "predictions_v29", report_filename)
    if bank_suffix:
        meta_path = os.path.join(BASE_DIR, "outputs", f"feature_banks_v29{bank_suffix}", "metadata.json")
    else:
        meta_path = os.path.join(BASE_DIR, "outputs", "feature_banks_v29", "metadata.json")

    if not os.path.exists(pred_path):
        logger.error(f"预测结果文件不存在: {pred_path}")
        logger.error("请先运行: python experiments/02_inference_auto_v29.py")
        sys.exit(1)

    if not os.path.exists(meta_path):
        logger.error(f"元数据文件不存在: {meta_path}")
        logger.error("请先运行: python experiments/01_build_feature_bank_v29.py")
        sys.exit(1)

    logger.info(f"读取预测结果: {pred_path}")
    logger.info(f"读取元数据:   {meta_path}")

    result = evaluate_v29(pred_path, meta_path)

    if result is None:
        logger.error("评估失败")
        sys.exit(1)

    log_section("v29 评估结果", logger)
    ...
    logger.info(f"\n评估报告已保存: {report_path}")
    log_finish("03_evaluate_v29", logger)
    ...
    print(f"\n对比 V23 基线 (IoU=0.5):")
    print(f"  V29 当前: TP={m05['tp']}, FP={m05['fp']}, F1={m05['f1']:.3f}, Precision={m05['precision']:.3f}, AUC(patch_mean)={result['image_auc_by_strategy']['patch_mean']['auc']:.3f}, AUC(region_max)={result['image_auc_by_strategy']['region_max']['auc']:.3f}")
    print("=" * 80)
