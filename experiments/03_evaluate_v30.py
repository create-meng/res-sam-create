...
if __name__ == '__main__':
    ...
    pred_filename = f"auto_predictions_v30{output_suffix}.json" if output_suffix else "auto_predictions_v30.json"
    report_filename = f"evaluation_report_v30{output_suffix}.json" if output_suffix else "evaluation_report_v30.json"
    pred_path   = os.path.join(BASE_DIR, "outputs", "predictions_v30", pred_filename)
    report_path = os.path.join(BASE_DIR, "outputs", "predictions_v30", report_filename)
    if bank_suffix:
        meta_path = os.path.join(BASE_DIR, "outputs", f"feature_banks_v30{bank_suffix}", "metadata.json")
    else:
        meta_path = os.path.join(BASE_DIR, "outputs", "feature_banks_v30", "metadata.json")
    ...
    print(f"\n对比 V23 基线 (IoU=0.5):")
    print(f"  V30 当前: TP={m05['tp']}, FP={m05['fp']}, F1={m05['f1']:.3f}, Precision={m05['precision']:.3f}, AUC(patch_mean)={result['image_auc_by_strategy']['patch_mean']['auc']:.3f}, AUC(region_max)={result['image_auc_by_strategy']['region_max']['auc']:.3f}")
    print("=" * 80)
