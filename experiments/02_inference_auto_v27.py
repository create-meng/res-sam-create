"""
Res-SAM v27 - Step 2: fully automatic inference
...
CONFIG = {
    ...
    "feature_bank_path": os.path.join(BASE_DIR, "outputs", "feature_banks_v27", "feature_bank_v27.pth"),
    "metadata_path":     os.path.join(BASE_DIR, "outputs", "feature_banks_v27", "metadata.json"),
    "test_data_dirs": {
    ...
    "output_dir":     os.path.join(BASE_DIR, "outputs", "predictions_v27"),
    "checkpoint_dir": os.path.join(BASE_DIR, "outputs", "checkpoints_v27"),
    ...
    # V27 固化基线：在 V25 二次筛选上继续收紧 mean_patch 门槛
...
def run_inference(config: dict) -> dict:
    ...
    logger = setup_global_logger(base_dir, "02_inference_auto_v27")
    ...
    log_section("Res-SAM V27：继承 V25 归档基线，准备继续优化检测指标", logger)
    ...
    if not os.path.exists(config["feature_bank_path"]):
        raise FileNotFoundError(f"Feature bank not found: {config['feature_bank_path']}\n"
                                f"请先运行 01_build_feature_bank_v27.py")
                    ...
                    if config.get("use_secondary_filter", 1) and len(pred_bboxes) > 0:
                        ...
                        for bbox_orig, bbox_resized, score in zip(pred_bboxes, pred_bboxes_resized, anomaly_scores):
                            ...
                            if area_orig < min_area_orig:
                                logger.debug(f"  [V27] 二次筛选移除框: 原图面积 {area_orig} < {min_area_orig}")
                                continue
                            if mean_patch_score < min_mean_patch:
                                logger.debug(
                                    f"  [V27] 二次筛选移除框: mean_patch_score {mean_patch_score:.4f} < {min_mean_patch:.4f}"
                                )
                                continue
                            if float(score) < min_box_score:
                                logger.debug(
                                    f"  [V27] 二次筛选移除框: box_score {float(score):.4f} < {min_box_score:.4f}"
                                )
                                continue
    ...
    output_filename = f"auto_predictions_v27{suffix}.json" if suffix else "auto_predictions_v27.json"
    ...
    log_finish("02_inference_auto_v27", logger)
...
if __name__ == "__main__":
    ...
    CONFIG = apply_layout_to_config_02_03(dict(CONFIG), BASE_DIR, "v27")
    bank_suffix = CONFIG.get("bank_suffix", "")
    if bank_suffix:
        CONFIG["feature_bank_path"] = os.path.join(
            BASE_DIR, "outputs", f"feature_banks_v27{bank_suffix}", f"feature_bank_v27{bank_suffix}.pth"
        )
        CONFIG["metadata_path"] = os.path.join(
            BASE_DIR, "outputs", f"feature_banks_v27{bank_suffix}", "metadata.json"
        )
    else:
        CONFIG["feature_bank_path"] = os.path.join(BASE_DIR, "outputs", "feature_banks_v27", "feature_bank_v27.pth")
        CONFIG["metadata_path"]     = os.path.join(BASE_DIR, "outputs", "feature_banks_v27", "metadata.json")
    CONFIG["output_dir"]        = os.path.join(BASE_DIR, "outputs", "predictions_v27")
    CONFIG["checkpoint_dir"]    = os.path.join(BASE_DIR, "outputs", "checkpoints_v27")
